...
// Deletes one user from database, by ID
...
// PasswordMatches ises the Go's bcrypt package to compare a user supplied password
//with the hash we have stored for a given user in the database/ If the password
// and has match, we return true; otherwise, we return a false.
...
//GetUserForToekn takes a token parameter, and uses the UserID field from that parameter
...
func (t *Token) AuthenticateToken(r *http.Request) (*User, error) {
	...
	// gets the plain text token from the header
	...
	if err != nil {
		return nil, errors.New("no macthing user found")
	}
